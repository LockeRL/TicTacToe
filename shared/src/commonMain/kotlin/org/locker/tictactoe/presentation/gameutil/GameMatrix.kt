...
abstract class GameMatrix<T>(
	initCell: () -> T,
	val dimension: Int = 3
) {
	private val matrix: List<List<T>> =
		List(dimension) { List(dimension) { initCell() } }
	...
	fun getAndUpdateBoardState(): BoardState {
		...
		val finishState = winState as? BoardState.Winner
			?: if (isFull()) BoardState.Draw else BoardState.InProgress
	...
	private fun checkSideDiagonal(): CellState {
		val list = List(dimension) { index ->
			getCellState(index, dimension - index - 1)
		}
		return checkList(list)
	}
